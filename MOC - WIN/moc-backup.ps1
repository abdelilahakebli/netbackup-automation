# Meaning of bpdbjobs form veritas documentation
# ===============================================
...
function Get-Backup {
    ...
    $data | foreach {
        ...
        # Checking for Backups that done 
        if($current -eq 0 -and $backupType -NotLike "*TEST_MSDP2*" -and $backup_state -eq 3) {
            $backup_counter = $backup_counter + 1
        }
        
        # Checking for Backups that Active 
        if($current -eq 0 -and $backupType -NotLike "*TEST_MSDP2*" -and $backup_state -eq 1) {
            $active_counter = $active_counter + 1
        }  
        
        # Checking for Backups that Failed 
        if(($current -eq 0 -or $current -eq 28) -and ($backupType -NotLike "*TEST_MSDP2*") -and ($backup_state -eq 3) -and ($backup_status -gt 1) )  {
            $failed_counter = $failed_counter + 1
        }

        # Checking for Backups that in Queud Status 
    ...
    # Print Data
    echo "DONE : $backup_counter - ACTIVE : $active_counter - FAILED : $failed_counter - QUEUD: $queud_counter " 
...
#nbdevquery -listdv -stype PureDisk -dp  ms08_cloud_somei -U
